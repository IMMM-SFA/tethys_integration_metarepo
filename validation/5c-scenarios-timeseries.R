...
# Clip the historical line at 2015 so it does not overlap the futures,
# which start at 2020.
...
read_annual_sum = function(path) {
  ...
  for (v in var_names) {
    ...
    total_per_year = total_per_year + apply(arr, length(dim(arr)), sum, na.rm = TRUE)
  }
...
parse_scenario = function(s) {
  if (s == "historical") {
    tibble(scenario = s, rcp = "historical", climate = NA_character_, ssp = NA_character_)
  } else {
...
demand = expand_grid(scenario = scenarios, sector = sectors) |>
  mutate(
    path = file.path(tethys_base, scenario, paste0(sector, "_", demand_type, ".nc"))
  ) |>
...
demand_all = bind_rows(demand, total_by_sector) |>
  mutate(
    sector = factor(sector, levels = c("Domestic", "Electricity", "Irrigation", "Total"))
  ) |>
  left_join(map_dfr(scenarios, parse_scenario), by = "scenario")

# Clip historical tail so it visually hands off to the future lines at 2020
...
p = ggplot() +
  ...
  scale_color_manual(values = scenario_colors, breaks = c("RCP4.5", "RCP8.5"), name = "RCP") +
