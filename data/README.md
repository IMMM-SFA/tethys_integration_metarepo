...
├── shapefiles/
│   └── HUC6/                  # WBD HUC6 polygons used by validation stages 4a and 5a
...
| `shapefiles/HUC6/HUC6.{shp,shx,dbf,prj}` | USGS Watershed Boundary Dataset HUC6 polygons (~221 MB, dominated by `.shp` geometry) | USGS National Hydrography Dataset / WBD: https://www.usgs.gov/national-hydrography/watershed-boundary-dataset. Read by `validation/4a-compare-tethys-usgs.R` and `validation/5a-paper-figures.R`; legacy local mount path `/Volumes/data/shapefiles/HUC6/HUC6.shp`. |
